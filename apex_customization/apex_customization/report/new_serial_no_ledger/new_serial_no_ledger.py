...
def execute(filters=None):
	...
	get_part_name(filters, data)
	data = filter_party(filters, data)
	return columns, data
...
def get_part_name(filters, data):
	if data:
		for i in data:
			if "party_type" in i:
				if i.party_type == "Supplier":
					party_name = frappe.db.get_value("Supplier",i.party,"supplier_name")
					i["party_name"] = party_name 
				elif i.party_type == "Customer":
					party_name = frappe.db.get_value("Customer",i.party,"customer_name")
					i["party_name"] = party_name
		
def filter_party(filters,data):
	if filters.get("party"):
		dod = [d for d in data if "party" in d and d['party'] == filters.get("party")]
		return dod
	else:
		return data